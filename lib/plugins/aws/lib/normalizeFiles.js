...
module.exports = {
  normalizeCloudFormationTemplate(template) {
    ...
    const sortByKey = ([key], [otherKey]) => key.localeCompare(otherKey);

    normalizedTemplate.Resources = _.fromPairs(
      Object.entries(normalizedTemplate.Resources).sort(sortByKey)
    );
    if (normalizedTemplate.Outputs) {
      normalizedTemplate.Outputs = _.fromPairs(
        Object.entries(normalizedTemplate.Outputs).sort(sortByKey)
      );
    }
